...
#ifndef CELLSHADERPASS_HLSL
#define CELLSHADERPASS_HLSL
...
*/
float _HalfShadowThreshold;
float _FullShadowThreshold;
float _ShadowAttenuationThreshold;
float3 Fragment(Varyings IN) : SV_Target
{
   // These macros are required for VR SPI compatibility
    ...
    if (NoL <= _FullShadowThreshold)
        toonLighting = .05;
    else if (NoL < _HalfShadowThreshold)
        toonLighting = .5;
    else
        toonLighting = 1;
    
    if (light.shadowAttenuation < _ShadowAttenuationThreshold)
        toonShadow = .05;
    else
        toonShadow = 1;
    ...
    return _Color * SAMPLE_TEXTURE2D(_ColorMap, sampler_linear_repeat, IN.uv) * toonLighting * light.color * toonShadow;
    //return _Color * SAMPLE_TEXTURE2D(_ColorMap, sampler_linear_repeat, IN.uv) * light.shadowAttenuation * light.color;
